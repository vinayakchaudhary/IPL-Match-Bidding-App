...
select bp.bidder_id, count(bid_status)Matches_won,no_of_matches,(count(bid_status)/no_of_matches)*100 Win_Percentage 
from IPL_BIDDER_POINTS bp
join IPL_BIDDING_DETAILS bd on
...
select ms.stadium_id,count(schedule_id) No_of_matches,s.stadium_name,s.city 
...
select u.stadium_id,count(*)n,sum(toss_match_same_team_win), (sum(toss_match_same_team_win)/count(*))*100 as win_per from 
(select ims.match_id,ims.stadium_id,im.team_id1,im.team_id2,im.TOSS_WINNER,im.MATCH_WINNER,
 if (im.TOSS_WINNER=im.MATCH_WINNER,1,0)toss_match_same_team_win from ipl_match im ,ipl_match_schedule ims 
 where im.MATCH_ID=ims.MATCH_ID)u group by ims.stadium_id ;
###################################
select ims.match_id,ims.stadium_id,im.team_id1,im.team_id2,im.TOSS_WINNER,im.MATCH_WINNER,
 if (im.TOSS_WINNER=im.MATCH_WINNER,1,0)toss_match_same_team_win from ipl_match im ,ipl_match_schedule ims 
 where im.MATCH_ID=ims.MATCH_ID;
